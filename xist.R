...
xcut_exact <- function(g, progress_bar=TRUE){
...
xist <- function(g, locmax_by_degree=FALSE, normalize=FALSE, progress_bar=TRUE){
  ...
  for(s in 2:N){
    ...
    # Computes the Vloc[s]-Vloc[t]-MinCut including its partition.
...
# Loads/Saves timearray for later use.
...
# Loads/Saves mat_rate and mat_ncut for later use.
